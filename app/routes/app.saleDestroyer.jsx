...
export const action = async ({ request }) => {
    ...
    if (intent === "remove") {
        ...
        // Remove tags from each selected product
        for (const productId of selectedProducts) {
            try {
                ...
                // Update with filtered tags (removing the sale tag)
                ...
                `, {
                    variables: {
                        input: {
                            id: productId,
                            tags: currentTags.filter(tag => tag !== saleTag)
                        }
                    }
                });
        ...
        return new Response(
            JSON.stringify({ 
                success: true,
                message: `Successfully removed sale tag from ${successCount} products${failedProducts.length > 0 ? `, ${failedProducts.length} failed` : ''}`,
